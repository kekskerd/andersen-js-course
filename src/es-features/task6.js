...
export const task6New = () => {
  ...
  const { name, value, role, cases: [{ id: firstCaseId }], } = userModule();
  const isActive = userModule().isActive ? userModule().isActive : false;

  return [name, value, role.name, isActive, firstCaseId];
};
